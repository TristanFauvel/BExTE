...
## 13 of the paper items - so an unfaithful directory is never chosen for you, only
## picked deliberately.
...
mod_replicate_server <- function(id, color_mode = NULL) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    ## botox, so no single directory could make all selected items and Step 3 had to
